...
function patrolMarkerGlowClass(connectionState) {
  switch (connectionState) {
    case CONNECTION_STATE.weak:
      return "patrol-marker-pin patrol-marker-glow patrol-marker-glow--weak";
    case CONNECTION_STATE.delayed:
      return "patrol-marker-pin patrol-marker-glow patrol-marker-glow--delayed";
    case CONNECTION_STATE.stale:
      return "patrol-marker-pin patrol-marker-glow patrol-marker-glow--plain";
    default:
      return "patrol-marker-pin patrol-marker-glow patrol-marker-glow--strong";
  }
}
...
export function createPatrolMarkerIcon(
  ...
  const glowClass = patrolMarkerGlowClass(connectionState);

  return L.divIcon({
    className: "patrol-marker",
    html: `<img src="${markerSrc}" alt="" class="${glowClass}" width="${sizePx}" height="${sizePx}" style="width:${sizePx}px;height:${sizePx}px;object-fit:contain;display:block;pointer-events:none;" />`,
