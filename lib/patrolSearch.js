...
/** True when query matches this patrol unit (same rules as search). */
export function patrolLocationMatchesSearch(location, query) {
  const q = String(query ?? "").trim();
  if (q.length < 2) return true;
  return scorePatrolMatch(location, q) > 0;
}

/** Keep only units matching the search query; returns all when query is empty/short. */
export function filterPatrolLocations(locations, query) {
  const list = Array.isArray(locations) ? locations : [];
  const q = String(query ?? "").trim();
  if (q.length < 2) return list;
  return list.filter((location) => scorePatrolMatch(location, q) > 0);
}
