...
export default function IsoFormModal({
  open,
  mode = "add",
  draft,
  saving = false,
  ...
  const canSave = String(draft.addressLocation ?? "").trim();
  const title = mode === "edit" ? "Edit ISO" : "Add ISO";
  const confirmTitle = mode === "edit" ? "Save changes?" : "Save ISO marker?";
    ...
    <>
      ...
      >
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="iso-form-title"
        >
          <div className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b border-border/60 bg-card px-5 py-4">
            <div>
              ...
              <p className="mt-1 text-xs text-muted">ISO</p>
            </div>
            <button
              type="button"
              aria-label="Close"
          ...
          >
            ...
            <label className="block text-xs font-medium text-muted">
              ...
              <textarea
                ...
                rows={2}
                placeholder="Input some relevant information"
                className="mt-1 w-full rounded-lg border border-border/70 bg-background/80 px-3 py-2 text-sm text-foreground outline-none focus:border-accent"
              />
            </label>
      ...
      <ConfirmDialog
        ...
        confirmLabel={mode === "edit" ? "Save changes" : "Save ISO"}
        cancelLabel="Back"
        confirming={saving}
