import { notifyLocationRequest, notifyAlarmStop } from "@/lib/mobile/fcmSend";
...
export async function closeLocationRequestBatch(admin, batchId) {
  if (!batchId) {
  ...
  const { data, error } = await admin.rpc("close_location_request_batch", {
    p_batch_id: batchId,
  });

  if (error) throw new Error(error.message);
  if (!data?.ok) {
    throw new Error(data?.error ?? "Could not close batch.");
  }

  const tokenIds = [
    ...new Set((pendingItems ?? []).map((item) => item.access_token_id).filter(Boolean)),
  ];
  await Promise.allSettled(
    tokenIds.map((accessTokenId) => notifyAlarmStop(admin, accessTokenId))
  );

  return getLocationRequestBatch(admin, batchId);
}
