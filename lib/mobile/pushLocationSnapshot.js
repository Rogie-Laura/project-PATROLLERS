...
export async function pushLocationSnapshot(admin, accessToken, profile, overrides = {}) {
  ...
  const dutyShifts = normalizeDutyShifts(
    overrides.duty_shifts ?? profile?.duty_shifts
  );
  const visibilityPoints = normalizeVisibilityPoints(
    overrides.visibility_points ?? profile?.visibility_points
  );
