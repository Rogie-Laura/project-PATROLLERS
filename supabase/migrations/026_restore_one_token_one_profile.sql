-- Restore 1-token-1-profile constraints after partial 025 apply broke ON CONFLICT.

-- Keep only the newest profile row per access token.
delete from public.mobile_device_profiles p
where p.id not in (
  select distinct on (access_token_id) id
  from public.mobile_device_profiles
  order by access_token_id, updated_at desc nulls last, id desc
);

drop index if exists public.mobile_device_profiles_token_device_uidx;

alter table public.mobile_device_profiles
  drop constraint if exists mobile_device_profiles_access_token_id_key;

alter table public.mobile_device_profiles
  add constraint mobile_device_profiles_access_token_id_key unique (access_token_id);
