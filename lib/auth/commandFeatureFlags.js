 ...
 * RCC/PCC/SCC follow toggles plus base role rules. System Administrator always
 * has all three toolbar actions regardless of toggles.
 */
export function canUseCommandFeature(role, featureKey, flags) {
  const normalized = normalizeRole(role);
  if (!normalized) return false;

  if (isSystemAdministrator(normalized)) {
    return (
      featureKey === COMMAND_FEATURE_KEYS.addCallResponse ||
      featureKey === COMMAND_FEATURE_KEYS.forceLocation ||
      featureKey === COMMAND_FEATURE_KEYS.generateReport
    );
  }
  ...
  if (featureKey === COMMAND_FEATURE_KEYS.forceLocation) {
    ...
    return isFeatureEnabled(flags, normalized, "forceLocation");
  }

  if (featureKey === COMMAND_FEATURE_KEYS.generateReport) {
    if (!isCommandCenterRole(normalized)) return false;
    return isFeatureEnabled(flags, normalized, "generateReport");
  }
