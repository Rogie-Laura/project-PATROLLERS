...
function hasJsonArray(value) {
  return Array.isArray(value) && value.length > 0;
}

/** Keep duty detail on Realtime GPS rows when the insert omits profile fields. */
export function mergeDutyDetailFromExisting(row, existing) {
  if (!existing) return row;

  const merged = { ...row };

  if (!hasJsonArray(merged.duty_shifts) && hasJsonArray(existing.duty_shifts)) {
    merged.duty_shifts = existing.duty_shifts;
  }
  if (
    !hasJsonArray(merged.visibility_points) &&
    hasJsonArray(existing.visibility_points)
  ) {
    merged.visibility_points = existing.visibility_points;
  }
  if (
    !hasJsonArray(merged.personnel_on_board) &&
    hasJsonArray(existing.personnel_on_board)
  ) {
    merged.personnel_on_board = existing.personnel_on_board;
  }
...
export function upsertLatestLocationRow(locations, row) {
  ...
  if (
    ...
    const nextRow = mergeDutyDetailFromExisting(
      lastSeenAt ? { ...row, last_seen_at: lastSeenAt } : row,
      existing
    );
    map.set(key, nextRow);
  }
