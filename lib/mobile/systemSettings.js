...
export async function getSystemSettings() {
  try {
    ...
    const { data, error } = await admin
      ...
      .select(
        "location_interval_seconds, directions_provider, incident_radius_rings, mobile_latest_version_code, mobile_min_version_code, mobile_latest_version_name, mobile_apk_download_url, mobile_update_required, mobile_release_notes, command_feature_flags, command_access_suspended"
      )
      .eq("id", "default")
      .maybeSingle();
    ...
    if (!error && data) {
      return {
        ...
        command_feature_flags: normalizeCommandFeatureFlags(
          data.command_feature_flags
        ),
        command_access_suspended: Boolean(data.command_access_suspended),
      };
    }
  } catch {
  ...
  return {
    ...
    command_access_suspended: false,
  };
}
...
export async function updateCommandFeatureFlags(flags, userId = null) {
  ...
  const { data, error } = await admin
    ...
    .upsert({
      id: "default",
      command_feature_flags: normalized,
      updated_at: new Date().toISOString(),
      updated_by: userId,
    })
    .select("command_feature_flags, updated_at")
    .single();

  if (error) throw error;
  return {
    ...data,
    command_feature_flags: normalizeCommandFeatureFlags(data.command_feature_flags),
  };
}

export async function updateCommandAccessSuspended(suspended, userId = null) {
  const admin = createAdminClient();
  ...
  const { data, error } = await admin
    ...
    .upsert({
      id: "default",
      command_access_suspended: Boolean(suspended),
      updated_at: new Date().toISOString(),
      updated_by: userId,
    })
    .select("command_access_suspended, updated_at")
    .single();
