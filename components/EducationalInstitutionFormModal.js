...
import { createEmptyPersonnelRow } from "@/lib/smartLocator/educationalInstitutions";

export default function EducationalInstitutionFormModal({
  open,
  mode = "add",
  draft,
  saving = false,
  ...
  useEffect(() => {
    ...
    const focusTimer = window.setTimeout(() => {
      dialogRef.current
        ?.querySelector("input:not([readonly]), textarea")
        ?.focus();
    }, 0);
  ...
  const isPollingCenter = Boolean(draft.isPollingCenter);
  ...
  const canSave =
    ...
    String(draft.estimatedStudents ?? "").trim() &&
    (!isPollingCenter ||
      (String(draft.numberOfVoters ?? "").trim() && hasCompletePersonnel));
  ...
  function setPollingCenter(next) {
    if (next) {
      onChange({
        isPollingCenter: true,
        personnel:
          personnel.length > 0 ? personnel : [createEmptyPersonnelRow()],
      });
      return;
    }
    onChange({
        ...
        >
          <div className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b border-border/60 bg-card px-5 py-4">
            <div>
              ...
              <p className="mt-1 text-xs text-muted">Educational Institutions</p>
            </div>
            <button
              type="button"
              aria-label="Close"
          ...
          >
            ...
            <label className="block text-xs font-medium text-muted">
              ...
              <input
                ...
                onChange={(event) =>
                  onChange({ principalSupervisor: event.target.value })
                }
                placeholder="Principal or school supervisor"
            ...
            <label className="block text-xs font-medium text-muted">
              ...
              <input
                value={draft.estimatedStudents ?? ""}
                onChange={(event) =>
                  onChange({ estimatedStudents: event.target.value })
                }
            ...
            <div>
              <p className="text-xs font-medium text-muted">
                Established as Polling Center?
              </p>
              <div className="mt-1.5 grid grid-cols-2 gap-2">
                <button
                  type="button"
                  disabled={saving}
                  onClick={() => setPollingCenter(true)}
                  className={`rounded-lg border px-3 py-2 text-sm font-medium transition disabled:opacity-50 ${
                    isPollingCenter
                      ? "border-accent bg-accent/15 text-foreground"
                      : "border-border/70 text-muted hover:bg-background/80 hover:text-foreground"
                  }`}
                >
                  Yes
                </button>
                <button
                  type="button"
                  disabled={saving}
                  onClick={() => setPollingCenter(false)}
                  className={`rounded-lg border px-3 py-2 text-sm font-medium transition disabled:opacity-50 ${
                    !isPollingCenter
                      ? "border-accent bg-accent/15 text-foreground"
                      : "border-border/70 text-muted hover:bg-background/80 hover:text-foreground"
            ...
            {isPollingCenter ? (
              <div className="space-y-3 border-t border-border/60 pt-3">
                ...
                <label className="block text-xs font-medium text-muted">
                  Number of Voters{" "}
                  <span className="font-normal">(based on COMELEC Data)</span>{" "}
                  <span className="text-accent">*</span>
                  <input
                    value={draft.numberOfVoters ?? ""}
                    onChange={(event) =>
                      onChange({ numberOfVoters: event.target.value })
                    }
