...
export async function releaseMobileDeviceBinding(rawToken, deviceId) {
  const token = String(rawToken ?? "").trim();
  const normalized = normalizeDeviceId(deviceId);
  if (!token || !normalized) {
    return { ok: false, error: "Missing token or device id." };
  }

  const admin = createAdminClient();
  const { data, error } = await admin.rpc("release_mobile_device_binding", {
    p_token: token,
    p_device_id: normalized,
  });

  if (error) {
    throw new Error(error.message ?? "Could not release token binding.");
  }
