...
export function analyzeCordon({ incidentLat, incidentLon, osmNodes, osmWays }) {
  ...
  const escapeNote =
    escapeRoutes.length === 0
      ? " Escape road lines were not loaded (map service busy); use the intersection list and local knowledge."
      : " Orange dashed lines show possible outward escape corridors.";
  ...
  const summary =
    rankedCheckpoints.length === 0
      ...
      : `Suggested ${rankedCheckpoints.length} cordon point(s) (${highCount} high priority) within ~5 km.${escapeNote} Confirm with field units before blocking roads.`;
