...
import { notifyDispatchAlert, notifyAlarmStop } from "@/lib/mobile/fcmSend";
...
export async function cancelAllActiveDispatches(admin, callResponseId) {
...
export async function createCallResponseDispatches(
  ...
  await Promise.allSettled(
    dispatches.map((entry) =>
      notifyDispatchAlert(admin, entry.accessTokenId, entry.id)
    )
  );
...
export async function createSingleUnitDispatch(
  ...
  await notifyDispatchAlert(admin, dispatch.accessTokenId, dispatch.id);
