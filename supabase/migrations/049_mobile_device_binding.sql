-- One access token bound to one mobile install while the duty phone is online.
-- Allows re-bind after ~15 minutes without heartbeat (replaced phone / cleared app data).
...
create index if not exists mobile_device_profiles_bound_device_idx
  on public.mobile_device_profiles (bound_device_id)
...
begin
  ...
  if v_device is null then
    ...
    if v_bound is not null and v_bound <> '' then
...
revoke all on function public.resolve_mobile_device_binding(uuid, text, boolean) from public;
grant execute on function public.resolve_mobile_device_binding(uuid, text, boolean) to anon, authenticated;
