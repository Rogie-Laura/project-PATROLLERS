...
export async function PATCH(request, { params }) {
  ...
  if (!id) {
    return NextResponse.json({ error: "Missing ISO marker id." }, { status: 400 });
  }
  ...
  try {
    ...
    if (!existing) {
      return NextResponse.json({ error: "ISO marker not found." }, { status: 404 });
    }
    ...
    if (body?.remarks != null) {
      update.remarks = String(body.remarks).trim() || null;
    }
    ...
    return NextResponse.json(
      { error: "Could not update ISO marker." },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    const { error: dbError } = await admin.from("iso").delete().eq("id", id);
    if (dbError) throw dbError;

    return NextResponse.json({ ok: true });
  } catch {
    return NextResponse.json(
      { error: "Could not delete ISO marker." },
      { status: 500 }
    );
  }
}
